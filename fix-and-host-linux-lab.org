...
** Linux Lab
   ...
   12. Now when we try to register the user
       ...
       #+END_EXAMPLE
       - Efforts ::
		   1. Posted question at [[http://stackoverflow.com/questions/37480782/ldapadduser-command-throws-could-not-guess-current-user][stackoverflow]]
		   2. After reading /usr/sbin/ldapadduser and
                      /usr/share/ldapscripts/runtime scripts I found
                      that the $USER variable is set using =logname=
                      command. But executing =logname= command returns
                      no name as follows:
		      #+BEGIN_EXAMPLE
		      $ logname
		      logname: no login name
		      #+END_EXAMPLE
		      - Replace =USER=$(logname 2>/dev/null)= with
                        =USER=$(whoami 2>/dev/null)=. refer:
                        http://stackoverflow.com/questions/18017311/bash-script-using-ldapscripts-package-does-not-work-from-cron
		      - Grant read privileges to ldapscripts.passwd
                        file.
                        #+BEGIN_EXAMPLE
		        chmod +r ldapscripts.passwd
		        #+END_EXAMPLE
		      - Now when add user it throws
			#+BEGIN_EXAMPLE
